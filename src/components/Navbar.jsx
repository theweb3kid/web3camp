...
const Navbar = ({scrollToTarget}) => {
  return (
    ...
    <div className="rounded-lg  flex flex-col md:flex-row items-start justify-start p-2 gap-[14px]">
      ...
      <div className=" flex flex-row items-center justify-center p-2">
        <div  onClick={()=>scrollToTarget('Web3Camp')} className="relative tracking-[0.03em] font-semibold">
          Web3Camp
        </div>
      </div>
      <div className=" flex flex-row items-center justify-center p-2">
        <div onClick={()=>scrollToTarget('Timeline')} className="relative tracking-[0.03em] font-semibold">
          Timeline
        </div>
      </div>
      <div className=" flex flex-row items-center justify-center p-2">
        <div onClick={()=>scrollToTarget('Curriculum')} className="relative tracking-[0.03em] font-semibold">
          Curriculum
        </div>
      </div>
      <div className=" flex flex-row items-center justify-center p-2">
        <div onClick={()=>scrollToTarget('Team')} className="relative tracking-[0.03em] font-semibold">Team</div>
      </div>
    </div>
    <div className=" flex shrink-0 flex-col md:flex-row items-start justify-center gap-[16px] text-sm text-white font-open-sans">
      <div className="rounded-lg bg-mediumslateblue-100 shrink-0 flex flex-row items-start justify-start py-3 px-4">
        <b className="relative tracking-[0.03em]">Register now</b>
      </div>
      <div className="rounded-lg bg-white shrink-0 flex flex-row items-start justify-start py-3 px-4 text-mediumslateblue-100 border-[1px] border-solid border-mediumslateblue-100">
